...
plotCellCR <- function(btcr, scrna, group) {
    ...
    data <- dplyr::group_by(data, group) %>% dplyr::summarise(percent = sum(CR) / length(CR))
    gg <- ggplot(data, aes(x = group, y = percent)) +
        geom_bar(stat = "identity", fill = "#8BC0C9") +
        scale_y_continuous(labels = scales::percent, limits = c(0, round(max(data$percent + 0.1), digits = 2))) +
        theme_classic() +
        labs(x = "", y = ylab)
    gg
}
...
plotCloneState <- function(btcr, scrna, group.by, split.by = NULL, status = c("status", "status2")) {
    ...
    colormap <- setNames(scales::hue_pal()(length(status_uniq)), nm = status_uniq)
    ...
    reformat <- function(dt) {
        dt <- reshape2::dcast(dt, x ~ fill)
        dt[, -1] <- dt[, -1] / sum(dt[, -1])
        reshape2::melt(dt, variable.name = "fill", value.name = "percent")
    }
...
barPointError <- function(data, x, y, fill, point = TRUE, errbar = TRUE) {
    ...
    data_mean <- dplyr::group_by(newdata, fill, x) %>%
        dplyr::summarise(mean = mean(y), se = se(y))
...
plotGeneBar <- function(btcr, group, sample = "sample", gene = c("v", "j", "c"),
    ...
    count_list <- lapply(count_list, function(innercount) {
        innercount <- innercount %>%
            dplyr::group_by(group, sample, gene) %>%
    ...
    for (gene_name in names(count_list)) {
        ...
        if (dtype == "percentage") plist[[gene_name]] <- plist[[gene_name]] + scale_y_continuous(labels = scales::percent)
        if (rotate) plist[[gene_name]] <- plist[[gene_name]] + theme(axis.text.x = element_text(angle = 45, hjust = 1))
    }

    if (combine && isoform) {
        patchwork::wrap_plots(plist, ncol = ncol, guides = "collect")
    } else if (!isoform) {
        # not isoform
        plist[[1]] + ggtitle("")
    } else {
...
#' @param btcr A BTCR object.
#' @param scrna The Single Cell RNAseq Seurat object corresponded to the BCR or TCR.
#' @param colorby can be "CR", "cdr3s_aa", "cdr3s_nt". If "CR", only plot whether a cell has a TCR or BCR. If "cdr3s", use topn or highlight to limit the clonotypes.
...
#' @export
plotCloneDim <- function(btcr, scrna, colorby = c("CR", "cdr3s_aa", "cdr3s_nt"), topn = 5, highlight = NULL, ...) {
    ...
    if (colorby == "CR") {
        ...
        colors <- setNames(
            c("gray", scales::hue_pal()(1)),
            nm = paste0(c("No ", ""), CR)
        )
        Seurat::DimPlot(scrna, group.by = "clonedim", cols = colors, ...)
    } else {
        ...
        colors <- setNames(scales::hue_pal()(length(colorbydata)), nm = colorbydata)

        Seurat::DimPlot(scrna, cells.highlight = celldata, cols.highlight = colors, ...)
    }
}
...
#' @export
plotCdr3 <- function(btcr, group, dtype = c("percentage", "count"), topn = 10, ncol = 1, combine = TRUE) {
    ...
    data_list <- lapply(data_list, function(dt) {
        ...
        dt <- dplyr::arrange(dt, dplyr::desc(count)) %>%
            dplyr::mutate(cdr3_grp = paste0(cdr3, "_", group)) %>%
            dplyr::group_by(group) %>%
            dplyr::top_n(n = topn, wt = count)
    ...
    plist <- lapply(names(data_list), function(nm) {
        ...
        if (dtype == "percentage") gg <- gg + scale_y_continuous(label = scales::percent)
        ggrob <- ggplot2::ggplotGrob(gg)
...
#' @return ggplot
#' @export
plotVJpair <- function(btcr, group, dtype = c("count", "percentage"), topn = 10, ncol = 1, position = c("right", "left", "bottom", "top")) {
    dtype <- match.arg(dtype)
    ...
    data <- dplyr::group_by(data, group, v_gene, j_gene) %>%
        dplyr::summarise(count = dplyr::n()) %>%
        dplyr::group_by(group) %>%
        dplyr::mutate(percentage = count / sum(count)) %>%
        dplyr::top_n(n = topn, wt = count)
...
plotSharedCloneDim <- function(btcr, scrna, values, dtype = c("share", "unique"), ncol = 1, ...) {
    ...
    plist <- lapply(overlaps, function(ov) {
        ...
        if (length(clono_ids) == 0 || is.null(clono_ids2)) {
            ...
            plt <- Seurat::DimPlot(scrna, cells.highlight = cells, cols.highlight = "gray", ...) + Seurat::NoLegend()
        } else {
            ...
            else colors <- setNames(scales::hue_pal()(length(cells)), nm = names(cells))
            plt <- Seurat::DimPlot(scrna, cells.highlight = cells, cols.highlight = colors, ...)
        }
